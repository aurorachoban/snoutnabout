...
export default function AddProductForm({ onAdded }) {
  ...
  // --- Variant helpers ---
  function addVariant() { setVariants((v) => [...v, { label: "", price: "" }]); }
  ...
  // --- Highlight helpers ---
  function addHighlight() { setHighlights((h) => [...h, { name: "", emoji: "", benefit: "" }]); }
  function updateHighlight(i, key, value) {
    setHighlights((h) => h.map((item, idx) => idx === i ? { ...item, [key]: value } : item));
  }
  function removeHighlight(i) { setHighlights((h) => h.filter((_, idx) => idx !== i)); }

  // --- Breakdown helpers ---
  function addBreakdown() { setBreakdown((b) => [...b, { name: "", pct: "" }]); }
  function updateBreakdown(i, key, value) {
    setBreakdown((b) => b.map((item, idx) => idx === i ? { ...item, [key]: value } : item));
  }
  function removeBreakdown(i) { setBreakdown((b) => b.filter((_, idx) => idx !== i)); }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setForm({ name: "", category: "dog", type: "food", price: "", description: "", featured: false, inStock: true, features: "", ingredientsFull: "", imageUrl: "" });
      setVariants([]);
      setHighlights([]);
      setBreakdown([]);
      onAdded?.();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Product Image */}
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        <h3 className="font-black text-gray-900 text-lg">Product Image</h3>
        <p className="text-xs text-gray-400">Enter the path to an image already in the <code className="bg-gray-100 px-1 rounded">/public/products/</code> folder, or any full URL.</p>
        <div>
          ...
          <input
            ...
            placeholder="/products/dog-food.jpg"
        ...
        {variants.map((v, i) => (
          <div key={i} className="flex gap-3 items-center">
            <input
              ...
              className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-pink-400 transition-colors bg-white"
            />
            <input
              type="number"
              value={v.price}
              ...
              min="0"
              step="0.01"
              className="w-28 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-pink-400 transition-colors bg-white"
            />
            <button type="button" onClick={() => removeVariant(i)} className="text-gray-300 hover:text-red-400 transition-colors font-bold text-lg">✕</button>
          </div>
        ))}
      </section>
          ...
          <div className="space-y-2">
            {breakdown.map((b, i) => (
              <div key={i} className="flex gap-3 items-center">
                ...
                <button type="button" onClick={() => removeBreakdown(i)} className="text-gray-300 hover:text-red-400 transition-colors font-bold text-lg">✕</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {highlights.map((h, i) => (
              <div key={i} className="flex gap-3 items-center flex-wrap">
                <input type="text" value={h.name} onChange={(e) => updateHighlight(i, "name", e.target.value)} placeholder="Name (e.g. Sweet Potato)" className={`${inputClass} flex-1`} />
                <input type="text" value={h.benefit} onChange={(e) => updateHighlight(i, "benefit", e.target.value)} placeholder="Benefit (e.g. High in fibre)" className={`${inputClass} flex-1`} />
                <button type="button" onClick={() => removeHighlight(i)} className="text-gray-300 hover:text-red-400 transition-colors font-bold text-lg">✕</button>
              </div>
            ))}
          </div>
        </div>
